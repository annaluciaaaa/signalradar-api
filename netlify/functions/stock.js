...
function get(url,cb){https.get(url,{headers:{“User-Agent”:“node”,“Accept”:“application/json”}},function(r){var d=””;r.on(“data”,function(c){d+=c});r.on(“end”,function(){try{cb(null,JSON.parse(d))}catch(e){cb(e)}})}).on(“error”,cb)}
...
var err=function(){done(null,{statusCode:500,headers:h,body:”{"error":"err"}”})};
...
if(e)return err();
try{var r=j.chart.result[0],out=r.timestamp.map(function(ts,i){return{date:new Date(ts*1000).toISOString().slice(0,10),c:r.indicators.quote[0].close[i],v:r.indicators.quote[0].volume[i]||0}}).filter(function(x){return x.c>0});ok({ticker:t2,data:out})}catch(e2){err()}
});
}else{
ok({ok:1});
}
